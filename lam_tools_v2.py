...
class Lamina:
    ...
    def __init__(self, tk, theta, E11, E22, nu12, G12, a11, a22,
                 b11, b22):
        self.ID = 0
        self.tk = tk
        self.z = 0
    ...
    def Qk_bar(self):
        ...
        return linalg.matmul(linalg.matmul(T_inv, Q), T)
...
class Laminate(list):
    ...
    def __lamUpdate_(self):
        ...
        A_star = linalg.inv(self.A)
        B_star = linalg.matmul(self.A_star, self.B)
        C_star = linalg.matmul(self.B, self.A_star)
        D_star = self.D - linalg.matmul(linalg.matmul(self.B,
                                                      linalg.matmul(
                                                          self.A_star,
                                                          self.B)))

        A_prime = A_star - linalg.matmul(B_star,
                                         linalg.matmul(linalg.inv(D_star),
                                                       C_star))
        B_prime = linalg.matmul(B_star, linalg.inv(D_star))
        C_prime = - linalg.matmul(linalg.inv(D_star), C_star)
        D_prime = linalg.inv(D_star)
        ...
        for ply in self:
            self._N_T += linalg.matmul(ply.Qk_bar, ply.alpha_k) * self.dT * \
                ply.tk  # See Staab Eq 6.25
            self._M_T += linalg.matmul(ply.Qk_bar, ply.alpha_k) * self.dT * \
                ply.tk * ply.z  # See Staab Eq 6.26
            self._N_H += linalg.matmul(ply.Qk, ply.beta_k) * self.M_bar * \
                ply.tk  # See Staab Eq 6.29
            self._M_H += linalg.matmul(ply.Qk, ply.beta_k) * self.M_bar * \
        ...
        self.epsilon_0, self.kappa_0 = np.vsplit(
            linalg.matmul(self.ABD_prime, np.vstack(_N_hat, _M_hat, 2)))

        # calculate on-axis ply strains and stresses (See Staab Eq 6.37-6.38)
        for ply in self:
            ply.epsilon_k = linalg.matmul(ply.T_e, (self.epsilon_0 +
                                                    self.z * self.kappa_0))

            ply.sigma_k = linalg.matmul(ply.Qk, (ply.epsilon_k -
                                                 ply.alpha_k * self.dT -
